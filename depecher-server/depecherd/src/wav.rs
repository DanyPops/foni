/// WAV encode/decode utilities for depecherd.
/// Decode is thin wrapper over depecher-analyse; encode uses hound.
use depecher_analyse::wav::WavData;
use hound::{SampleFormat, WavSpec, WavWriter};

pub use depecher_analyse::decode_wav;
...
pub fn cap_wav(bytes: &[u8], max_secs: f32) -> Vec<u8> {
    let Ok(wav) = depecher_analyse::decode_wav(bytes) else {
        return bytes.to_vec();
    };
...
mod tail_tests {
    ...
    fn cap_wav_truncates_long_audio() {
        ...
        let dec = depecher_analyse::decode_wav(&capped).unwrap();
