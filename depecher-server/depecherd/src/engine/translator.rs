...
// Types and pure helpers live in depecher-client so depecherctl can reuse them.
pub use depecher_client::commentary::{
...
///
/// Delegates to `depecher_client::commentary::ollama_commentary` with the emotion
...
pub async fn ollama_commentary(
    sentence: &str,
    bias: WordBias,
    seed: &lexicon::CharacterSeed,
    url: &str,
    model: &str,
    timeout_ms: u64,
) -> Result<CommentaryResult, String> {
    let emotion = match bias {
        WordBias::Aggressive => "angry, aggressive",
        WordBias::Commiseration => "sympathetic, commiserating",
        WordBias::Mockery => "mocking, sarcastic",
    ...
    let client_seed = depecher_client::commentary::CharacterSeed {
        persona: &seed.persona,
        expressions: &expressions,
    };
    let result = depecher_client::commentary::ollama_commentary(
        sentence,
        emotion,
        &client_seed,
        url,
...
mod tests {
    ...
    // parse_commentary_response / apply_commentary tests live in depecher-client.

    #[test]
    fn character_seed_loads_from_compiled_yaml() {
        let seed = lexicon::character_seed();
        assert!(!seed.persona.is_empty(), "persona should be set");
        assert!(
            seed.expressions.len() >= 6,
            "seed should have at least 6 expressions, got {}",
            seed.expressions.len()
        );
    }

    #[test]
    fn character_seed_expressions_are_nonempty_strings() {
        for expr in &lexicon::character_seed().expressions {
