...
use super::engine_config::DepecherConfig;
...
async fn handle_socket(socket: WebSocket, app_state: AppState) {
    ...
    let defaults = DepecherConfig::default();
    let mut config = DepecherConfig {
        dry_run: std::env::var("DEPECHER_DRY_RUN")
            .map(|v| v == "1")
            .unwrap_or(false),
        ollama_url: std::env::var("DEPECHER_OLLAMA_URL").unwrap_or(defaults.ollama_url.clone()),
        ollama_model: std::env::var("DEPECHER_OLLAMA_MODEL")
            .unwrap_or(defaults.ollama_model.clone()),
        ..defaults
    };
    if let Ok(mode) = std::env::var("DEPECHER_STRESS") {
        use std::str::FromStr;
        config.stress_mode = super::stress::StressMode::from_str(&mode).unwrap_or_default();
    }
    if let Ok(backend) = std::env::var("DEPECHER_TRANSLATE") {
        use std::str::FromStr;
...
async fn prepare_and_enqueue(
    ...
    let commentary_task = if !config.dry_run
        ...
        && translated.len() > 2
    {
        use depecher_client::commentary::Placement;
        ...
        let handle: tokio::task::JoinHandle<Result<_, String>> = tokio::spawn(async move {
            ...
            let r = depecher_client::commentary::ollama_commentary(
                &text_for_commentary,
                emotion_str,
                &client_seed,
                &url,
