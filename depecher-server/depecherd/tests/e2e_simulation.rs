...
fn sine_wav(freq: f32, duration_secs: f32, sample_rate: u32) -> Vec<u8> {
    ...
    let samples: Vec<f32> = (0..n)
        ...
        .collect();
    depecherd::wav::encode_wav(&samples, sample_rate).expect("infallible")
}
...
async fn start_depecherd() -> (String, String) {
    start_depecherd_with_synth(depecherd::engine::synth_backend::mock_backend()).await
}

async fn start_depecherd_with_synth(
    synth: depecherd::engine::synth_backend::SharedSynth,
) -> (String, String) {
    // Start a mock Ollama so translation doesn't need a real LLM.
    let ollama_url = start_mock_ollama().await;
    std::env::set_var("DEPECHER_OLLAMA_URL", &ollama_url);
    std::env::set_var("DEPECHER_DRY_RUN", "0");

    let listener = TcpListener::bind("127.0.0.1:0").await.unwrap();
    let addr = listener.local_addr().unwrap();
    let port = addr.port();
    let app = depecherd::build_router_with_synth(synth).await;
    tokio::spawn(async move { axum::serve(listener, app).await.unwrap() });
    (
...
async fn full_pipeline_delta_to_playing() {
    ...
    // 2. Configure depecherd to use mocks
    std::env::set_var("DEPECHER_TTS_URL", &tts_url);
    std::env::set_var("DEPECHER_OLLAMA_URL", &ollama_url);

    // 3. Start depecherd
    let (ws_url, _http_url) = start_depecherd().await;
...
async fn full_pipeline_http_synthesize_with_mock_tts() {
    ...
    let (_ws_url, http_url) = start_depecherd().await;
...
async fn full_pipeline_emotion_then_synthesis() {
    ...
    let (ws_url, _) = start_depecherd().await;
    let (mut ws, _) = connect_async(&ws_url).await.expect("WS connect");
...
async fn concurrent_synthesis_doesnt_deadlock() {
    let (ws_url, _http) = start_depecherd().await;
...
/// Instead it must call the TTS backend (DEPECHER_TTS_URL) directly.
/// FAILS before FON-TSK-197: synthesize_local always calls localhost regardless of DEPECHER_TTS_URL.
#[tokio::test]
async fn synth_calls_tts_url_not_self() {
    ...
    // (because the WS server is busy). If it calls DEPECHER_TTS_URL directly it succeeds.
...
async fn ten_concurrent_connections_with_realistic_delay() {
    use depecherd::engine::synth_backend::MockSynthBackend;
    use std::sync::Arc;

    let synth = Arc::new(MockSynthBackend::with_delay(300));
    let (ws_url, _) = start_depecherd_with_synth(synth).await;
