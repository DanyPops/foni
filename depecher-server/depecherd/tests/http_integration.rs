/// http_integration — depecherd HTTP routes against a real bound server.
///
...
///
/// cargo test -p depecherd --test http_integration -- --nocapture
...
fn silence_wav(sample_rate: u32, duration_ms: u32) -> Vec<u8> {
    ...
    depecherd::wav::encode_wav(&vec![0.0f32; n], sample_rate).unwrap()
}
...
fn sine_wav(sample_rate: u32, duration_ms: u32) -> Vec<u8> {
    ...
    let samples: Vec<f32> = (0..n)
        ...
        .collect();
    depecherd::wav::encode_wav(&samples, sample_rate).unwrap()
}

/// RMS of decoded WAV bytes.
fn wav_rms(bytes: &[u8]) -> f32 {
    let wav = depecherd::wav::decode_wav(bytes).unwrap();
    let s = &wav.samples;
