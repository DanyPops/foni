...
///
/// cargo test -p depecherd --test tts_contract -- --nocapture
...
fn sine_wav() -> Vec<u8> {
    ...
    let samples: Vec<f32> = (0..n)
        ...
        .collect();
    depecherd::wav::encode_wav(&samples, sr).expect("infallible")
}
...
async fn contract_success_returns_wav() {
    ...
    std::env::set_var("DEPECHER_TTS_URL", format!("{base_url}/synthesize"));
    std::env::remove_var("DEPECHER_TTS_TOKEN");
...
async fn contract_no_url_returns_error() {
    std::env::remove_var("DEPECHER_TTS_URL");
