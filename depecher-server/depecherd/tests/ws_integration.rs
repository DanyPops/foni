...
///
/// cargo test -p depecherd --test ws_integration -- --nocapture
...
async fn start_server() -> String {
    ...
    let synth = depecherd::engine::synth_backend::mock_backend();
...
async fn synth_backend_is_injected_not_self_called() {
    // After the refactor, the server should accept a DEPECHER_TTS_URL env var
    // pointing to a mock TTS (not itself). Verify synthesis succeeds without
    // calling localhost:5050/synthesize.
    // dry_run is set per-connection by connect(); DEPECHER_TTS_URL is irrelevant in dry_run.
    let url = start_server().await;
    let mut ws = connect(&url).await;
    send_msg(&mut ws, json!({"type": "delta", "text": "Test. "})).await;
