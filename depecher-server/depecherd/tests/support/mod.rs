...
//!
//! [`StreamFixture`] wraps a WebSocket connection to depecherd and provides
...
/// Spawn a depecherd server with [`MockSynthBackend`] on a random port.
///
/// No env-var side effects — `DEPECHER_DRY_RUN` is explicitly cleared so
/// per-connection `set_config {dry_run}` controls the mode.
pub async fn start_mock_server() -> String {
    start_slow_mock_server(0).await
}

/// Spawn a depecherd server whose [`MockSynthBackend`] artificially delays
...
pub async fn start_slow_mock_server(delay_ms: u64) -> String {
    use std::sync::Arc;
    std::env::remove_var("DEPECHER_DRY_RUN");
    let listener = TcpListener::bind("127.0.0.1:0").await.unwrap();
    let addr = listener.local_addr().unwrap();
    let synth: depecherd::engine::synth_backend::SharedSynth = if delay_ms == 0 {
        depecherd::engine::synth_backend::mock_backend()
    } else {
        Arc::new(depecherd::engine::synth_backend::MockSynthBackend::with_delay(delay_ms))
    };
...
/// Controlled WS session against a depecherd server.
