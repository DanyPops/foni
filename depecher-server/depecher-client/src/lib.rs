//! depecher-client — typed async HTTP client for depecherd.
//!
...
//!
//! All methods are async. Construct one `DepecherClient` and share it (Clone).
...
/// Async HTTP client for depecherd. Cheap to clone — shares the connection pool.
#[derive(Clone)]
pub struct DepecherClient {
    base: String,
    http: reqwest::Client,
}
