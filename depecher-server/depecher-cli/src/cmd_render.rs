...
fn resolve_shade(colorset: &dyn Colorset, name: &str) -> (f32, f32, f32) {
    ...
    if let Some(result) = depecherd::engine::expression_palette::resolve_labels(name) {
        return result;
    }
...
pub fn cmd_render(
    ...
    let client = Arc::new(depecher_client::DepecherClient::new(server));
    ...
    let wavs = rt.block_on(collect_parallel(beats_arc.len(), concurrency, move |i| {
        ...
        async move {
            let t0 = Instant::now();
            let mut req = depecher_client::SynthRequest::new(&beat.text);
            req.voice = voice;
            req.model = model;
            req.dsp = false;
