...
pub fn cmd_tune(
    ...
    while i < maquettes.len() {
        let m = &maquettes[i];
        let wav_path =
            std::env::temp_dir().join(format!("depecherctl_tune_{}.wav", m.name.replace(' ', "_")));
...
pub fn cmd_tune_auto(
    ...
    let gap_score = |knobs: &Knobs| -> Option<f32> {
        ...
        let tmp = std::env::temp_dir().join("depecherctl_auto_tune.wav");
...
pub fn cmd_test_policy(
    ...
    let engine = match depecherd::quality::dsp::policy::PolicyEngine::load(script) {
        Some(e) => e,
        None => {
    ...
    let cfg = depecherd::quality::dsp::controller::ControllerConfig::default();
    let base = depecherd::quality::dsp::SmoothingOptions::default();

    let mut analysis = depecher_analyse::AnalysisResult {
