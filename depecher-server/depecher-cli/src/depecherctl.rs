...
#[derive(Parser)]
#[command(name = "depecherctl", about = "depecherd WAV factory", version)]
struct Cli {
    /// depecherd base URL
    #[arg(
        long,
        env = "DEPECHER_SYNTH_URL",
        default_value = "http://localhost:5051"
    )]
    server: String,
...
enum Cmd {
    ...
    Commentary {
        ...
        #[arg(
            long,
            env = "DEPECHER_OLLAMA_URL",
            default_value = "http://localhost:11434"
        )]
        ollama_url: String,
        /// Ollama model name.
        #[arg(long, env = "DEPECHER_OLLAMA_MODEL", default_value = "qwen3:1.7b")]
        model: String,
    ...
    tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| "depecherctl=info,depecherd=info,depecher_analyse=info".into()),
        )
        .with_target(false)
