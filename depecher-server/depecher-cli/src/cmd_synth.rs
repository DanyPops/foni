...
pub fn cmd_diagnose(server: &str, text: &str, model: &str) {
    ...
    for (label, desc, opts) in &stages {
        match process_request(server, &rvc_wav, opts.clone()) {
            Ok(wav) => {
                let path = std::env::temp_dir().join(format!("depecherctl_diag_{label}.wav"));
...
pub fn cmd_process(
    ...
    use depecher_analyse::{analyse, compute_gap, decode_wav, format_gap_table, TargetTensor};
...
pub fn cmd_listen(server: &str, text: &str, model: &str, dsp_variants: bool, play_ref: bool) {
    ...
    for (i, (label, _prosody, dsp)) in stages.iter().enumerate() {
        ...
        match result {
            Ok(bytes) => {
                let path = std::env::temp_dir().join(format!("depecherctl_listen_{i}.wav"));
...
pub fn cmd_mix(
    ...
    let mut tracks: Vec<super::tui::state::Track> = maquettes
        ...
        .map(|m| {
            let path = std::env::temp_dir()
                .join(format!("depecherctl_mix_{}.wav", m.name.replace(' ', "_")));
    ...
    for (slug, desc) in &[
        ...
        let path = std::env::temp_dir().join(format!("depecherctl_diag_{slug}.wav"));
        if path.exists() {
    ...
    // Pre-render un-rendered maquettes upfront.
    {
        ...
        if !to_render.is_empty() {
            ...
            let base_result = rt.block_on(client.synthesize(&depecher_client::SynthRequest {
                text: text.to_string(),
