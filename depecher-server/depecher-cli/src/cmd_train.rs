...
pub fn cmd_train(
    ...
    rt.block_on(async {
        ...
        if !follow {
            ...
            info!("Check status:  depecherctl train-status {call_id}");
            info!("Stream logs:   depecherctl train-logs {call_id}");
            info!("Cancel:        depecherctl train-cancel {call_id}");
            return;
        }
            ...
            Ok(lines) => {
                ...
                match modal_cloud::job_status(&mut client, &call_id).await {
                    ...
                    Ok(modal_cloud::JobStatus::Running) => {
                        info!("\n  ⏳ Still running. Check: depecherctl train-status {call_id}")
                    }
...
pub fn cmd_snapshot(server: &str, model: &str, ref_path: &std::path::Path) -> Result<(), String> {
    use depecher_analyse::{analyse, compute_gap, decode_wav, TargetTensor};
    use owo_colors::OwoColorize;
...
pub fn cmd_compare_models(
    ...
    let baseline: serde_json::Value = match std::fs::read_to_string(&baseline_path) {
        ...
        Err(_) => {
            return Err(format!(
                "No baseline at {}. Run: depecherctl snapshot {model} --vs {}",
...
pub fn cmd_tts_compare(phrase: &str) {
    ...
    rt.block_on(async {
        ...
        let token = std::env::var("DEPECHER_TTS_TOKEN").unwrap_or_default();
        ...
        match cb_res {
            Ok(resp) if resp.status().is_success() => {
                ...
                let path = "/tmp/depecherctl_compare_chatterbox.wav";
        ...
        match fs_res {
            Ok(resp) if resp.status().is_success() => {
                let wav = resp.bytes().await.unwrap_or_default();
                let path = "/tmp/depecherctl_compare_fish.wav";
                std::fs::write(path, &wav).ok();
        ...
        for i in 0..3 {
            ...
            match resp {
                Ok(r) if r.status().is_success() => {
                    ...
                    if i == 0 {
                        let path = "/tmp/depecherctl_bench.wav";
