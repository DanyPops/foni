...
//!
//! Hits the live depecherd → Chatterbox pipeline.
...
pub fn cmd_bench_roundtrip(server: &str, chunks: usize, parallel: bool) -> Result<(), String> {
    ...
    let t0 = Instant::now();
    let req = depecher_client::SynthRequest::new("test");
...
async fn bench_sequential(
    client: &depecher_client::DepecherClient,
    phrases: &[&str],
    ...
    for (i, phrase) in phrases.iter().enumerate() {
        ...
        let t0 = Instant::now();
        let req = depecher_client::SynthRequest::new(*phrase);
        let wav = client
            .synthesize(&req)
            .await
...
async fn bench_parallel(
    ...
    for (i, phrase) in phrases.iter().enumerate() {
        let client = depecher_client::DepecherClient::new(client.base_url());
    ...
    let mut results: Vec<(usize, Duration, Result<depecher_client::WavData, _>)> = Vec::new();
