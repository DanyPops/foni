use depecher_client::commentary::{apply_commentary, ollama_commentary, CharacterSeed};
...
fn load_seed(path: Option<&PathBuf>) -> (String, Vec<String>) {
    let yaml = path
        ...
        .or_else(|| {
            std::env::var("DEPECHER_LEXICON_PATH")
                .ok()
                .and_then(|p| std::fs::read_to_string(p).ok())
        });
...
mod tests {
    ...
    fn defaults_when_no_path_and_no_env() {
        // Ensure DEPECHER_LEXICON_PATH is not set for this test.
        std::env::remove_var("DEPECHER_LEXICON_PATH");
    ...
    fn reads_foni_lexicon_path_env() {
        ...
        write!(f, "{yaml}").unwrap();
        std::env::set_var("DEPECHER_LEXICON_PATH", f.path());
        let (persona, exprs) = load_seed(None);
        std::env::remove_var("DEPECHER_LEXICON_PATH");
    ...
    fn explicit_path_takes_priority_over_env() {
        ...
        std::env::set_var("DEPECHER_LEXICON_PATH", env_file.path());
