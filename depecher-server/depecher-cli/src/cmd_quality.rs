...
pub fn cmd_analyse(
    ...
    if let Some(ref_path) = vs {
        ...
        if show_timeline {
            ...
            // Tempo comparison
            let tc = depecher_analyse::tempo::compare(
                &ref_wav.samples,
                &wav.samples,
                ref_wav.sample_rate,
            );
...
pub fn cmd_compare(
    ...
    use depecher_analyse::{analyse_fast, decode_wav, transcribe};
    use rayon::prelude::*;
    use std::sync::Mutex;
    ...
    for path in &files {
        ...
        if !synth_path.exists() || !skip_transcribe {
            let req = depecher_client::SynthRequest {
                text: text.clone(),
                model: Some(model.to_string()),
                prosody: false,
                dsp: false, // raw RVC — same conditions as studio (no post-processing)
                ..depecher_client::SynthRequest::new(&text)
            };
    ...
    pairs.par_iter().for_each(|(s_path, y_path)| {
        let analyse = |p: &PathBuf| -> Option<(Row, Vec<f32>, u32)> {
            ...
            let (f0, f0s, _) = depecher_analyse::fast_f0_stats(&wav.samples, wav.sample_rate);
            let row = Row {
        ...
        if let (Some((mut sr, _, _)), Some((sy, _, _))) = (analyse(s_path), analyse(y_path)) {
            let score = depecher_analyse::naturalness::score(
...
pub fn cmd_sweep(
    ...
    use depecher_analyse::{analyse, compute_gap, decode_wav, spectral_timeline, TargetTensor};
    use owo_colors::OwoColorize;
    use tabled::{settings::Style, Table, Tabled};
    ...
    let metric_gap = |gap: &depecher_analyse::GapResult, name: &str| -> f32 {
        gap.rows
            .iter()
...
pub fn cmd_calibrate(server: &str, phrase: &str, ref_path: &PathBuf, model: &str) {
    use depecher_analyse::{analyse_fast, decode_wav};
    ...
    struct Metric {
        _name: &'static str,
        extract: fn(&depecher_analyse::AnalysisResult) -> f32,
    }
    let metrics = [
        Metric {
            _name: "brightness_hz",
...
pub fn cmd_energy(file: &std::path::Path, frame_ms: usize) -> Result<(), String> {
    let bytes = std::fs::read(file).map_err(|e| format!("{}: {e}", file.display()))?;
    let wav = depecher_analyse::decode_wav(&bytes).map_err(|e| format!("decode: {e}"))?;
