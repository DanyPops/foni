/**
 * Depecher TTS panel — opened by /tts with no arguments.
 *
 * ╭──────────── 🔊 Depecher ─────────────────╮
 * │                                        │
 * │  ▶ TTS          [ON ]  synth           │
...
export interface DepecherPanelState {
  enabled: boolean;
  muted: boolean;
  wsConnected: boolean;
  voice: string;
...
export interface DepecherPanelActions {
  toggleEnabled(): void;
  toggleMuted(): void;
  toggleMat(): void;
...
class DepecherPanel {
  ...
  constructor(
    private readonly stateGetter: () => DepecherPanelState,
    private readonly actions: DepecherPanelActions,
  ) {}
  ...
  render(_width: number): string[] {
    ...
    const title = bold("🔊 Depecher");
    const titleW = visibleWidth("🔊 Depecher");
...
export function openDepecherPanel(
  ctx: ExtensionContext,
  stateGetter: () => DepecherPanelState,
  actions: DepecherPanelActions,
): void {
  if (!ctx.hasUI) return;

  ctx.ui.custom(
    (tui, _theme, _keybindings, done) => {
      const panel = new DepecherPanel(stateGetter, actions);
