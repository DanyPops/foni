...
fn dtw_path(a: &[f32], b: &[f32]) -> Vec<(usize, usize)> {
    ...
    for i in 1..n { cost[i][0] = cost[i-1][0] + (a[i] - b[0]).abs(); }
    for j in 1..m { cost[0][j] = cost[0][j-1] + (a[0] - b[j]).abs(); }
    for i in 1..n {
        for j in 1..m {
    ...
    while i > 0 || j > 0 {
        if i == 0 { j -= 1; }
            ...
            if best == cost[i-1][j-1] { i -= 1; j -= 1; }
            else if best == cost[i-1][j] { i -= 1; }
            else { j -= 1; }
        }
        path.push((i, j));
    }
...
fn pearson(x: &[f32], y: &[f32]) -> f32 {
    ...
    let num: f64 = x.iter().zip(y).map(|(&a, &b)| (a as f64 - mx) * (b as f64 - my)).sum();
    let da:  f64 = x.iter().map(|&a| (a as f64 - mx).powi(2)).sum::<f64>().sqrt();
    let db:  f64 = y.iter().map(|&b| (b as f64 - my).powi(2)).sum::<f64>().sqrt();
    if da * db < 1e-12 { return 0.0; }
...
pub fn contour_correlation(reference: &[f32], synthesis: &[f32]) -> f32 {
    if reference.is_empty() || synthesis.is_empty() { return 0.0; }
...
pub fn compute_contour_correlations(
    ref_f0:      &[f32], ref_energy:  &[f32],
    syn_f0:      &[f32], syn_energy:  &[f32],
) -> (f32, f32) {
    (
...
mod tests {
    ...
    fn sine_contour(n: usize) -> Vec<f32> {
        (0..n).map(|i| (2.0 * PI * i as f32 / n as f32).sin()).collect()
    }
