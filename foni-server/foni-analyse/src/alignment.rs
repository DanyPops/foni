...
pub fn align(
    ...
    let pause_pairs: Vec<_> = pairs
        .iter()
        .filter(|p| p.label.starts_with("[pause"))
        .collect();
    let worst_pause_ratio = pause_pairs
        .iter()
        .map(|p| p.duration_ratio)
        .fold(f32::INFINITY, f32::min);
    let worst_pause_ratio = if worst_pause_ratio.is_infinite() {
        0.0
    } else {
        worst_pause_ratio
    };

    let word_pairs: Vec<_> = pairs
        .iter()
        .filter(|p| !p.label.starts_with("[pause"))
        .collect();
    let mean_rms_delta_db = if word_pairs.is_empty() {
        0.0
    } else {
...
pub fn format_alignment_table(cmp: &TimelineComparison) -> String {
    ...
    let mut lines = vec![
        format!(
            "{:<30} {:>8}  {:>8}  {:>8}  {:>6}",
            "Segment", "Ref(ms)", "Syn(ms)", "Ratio", "Status"
        ),
        sep.clone(),
    ];
    for p in &cmp.pairs {
        let status = if p.duration_ratio == 0.0 {
            ...
            if p.duration_ratio >= 0.5 {
                "✅ ok"
            } else if p.duration_ratio >= 0.3 {
                "🟡 short"
            } else {
                "🔴 collapsed"
            }
        } else {
            if p.duration_ratio >= 0.6 && p.duration_ratio <= 1.8 {
                "✅ ok"
            } else if p.duration_ratio < 0.6 {
                "🟠 too short"
            } else {
                "🟠 too long"
            }
...
mod tests {
    ...
    fn short_pause_low_ratio() {
        ...
        let cmp = align(&fixture(), &voiced, &pauses);
        assert!(
            cmp.worst_pause_ratio < 0.2,
            "ratio={}",
            cmp.worst_pause_ratio
        );
    }
}
