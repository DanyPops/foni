...
pub fn compute(samples: &[f32], sample_rate: u32) -> VoiceMetrics {
    ...
    if samples.len() < frame_size * 2 {
        return VoiceMetrics { hnr_db: 0.0, cpp: 0.0, jitter: 0.0, shimmer: 0.0 };
    }
...
fn compute_hnr(samples: &[f32], sample_rate: u32, frame_size: usize) -> f32 {
    ...
    if count == 0 { 0.0 } else { (hnr_sum / count as f64) as f32 }
}
...
fn compute_cpp(samples: &[f32], sample_rate: u32, frame_size: usize) -> f32 {
    ...
    while i + frame_size <= samples.len() {
        ...
        let windowed: Vec<f64> = frame.iter().zip(&hann).map(|(&s, &w)| s as f64 * w).collect();
        ...
        let log_power: Vec<f64> = {
            let n = frame_size;
            (0..n / 2).map(|k| {
                let re: f64 = windowed.iter().enumerate()
                    .map(|(t, &x)| x * (2.0 * PI * k as f64 * t as f64 / n as f64).cos())
                    .sum();
                let im: f64 = windowed.iter().enumerate()
        ...
        let n = log_power.len();
        let cepstrum: Vec<f64> = (0..n).map(|q| {
            log_power.iter().enumerate()
        ...
        if hi <= lo { i += hop; continue; }

        let peak = cepstrum[lo..=hi].iter().cloned().fold(f64::NEG_INFINITY, f64::max);
        // Linear baseline at the quefrency of the peak
        let slope = (cepstrum[hi] - cepstrum[lo]) / (hi - lo) as f64;
        let peak_q = cepstrum[lo..=hi].iter().enumerate()
    ...
    if count == 0 { 0.0 } else { (cpp_sum / count as f64) as f32 }
}
...
mod tests {
    ...
    fn pure_sine_high_hnr() {
        ...
        assert!(m.hnr_db > 5.0, "hnr_db={} — expected > 5 for pure sine", m.hnr_db);
    }
    ...
    fn pure_sine_low_jitter() {
        let samples = sine(200.0, 1.0, 22050);
        let m = compute(&samples, 22050);
        assert!(m.jitter < 0.05, "jitter={} — expected near zero for perfect sine", m.jitter);
    }
    ...
    fn pure_sine_low_shimmer() {
        let samples = sine(200.0, 1.0, 22050);
        let m = compute(&samples, 22050);
        assert!(m.shimmer < 0.05, "shimmer={} — expected near zero for pure sine", m.shimmer);
    }
