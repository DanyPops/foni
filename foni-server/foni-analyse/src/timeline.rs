...
pub enum SegmentKind { Voiced, Silence }
...
pub fn segment(samples: &[f32], sample_rate: u32) -> Timeline {
    ...
    if samples.is_empty() || frame_size == 0 {
        return Timeline { segments: vec![], total_duration_s: 0.0, pause_count: 0, silence_ratio: 0.0 };
    }
    ...
    while i < voiced.len() {
        let kind = voiced[i];
        let run_start = i;
        while i < voiced.len() && voiced[i] == kind { i += 1; }
        let run_end = i;
        ...
        segments.push(Segment {
            kind: if kind { SegmentKind::Voiced } else { SegmentKind::Silence },
            start_s,
            end_s,
            rms_db,
    ...
    let pause_count = segments.iter()
        .filter(|s| s.kind == SegmentKind::Silence && s.duration_s >= PAUSE_MIN_S)
        .count();
    let total_silence_s: f32 = segments.iter()
        .filter(|s| s.kind == SegmentKind::Silence)
        .map(|s| s.duration_s)
        .sum();
    let silence_ratio = if total_duration_s > 0.0 { total_silence_s / total_duration_s } else { 0.0 };

    Timeline { segments, total_duration_s, pause_count, silence_ratio }
}
...
pub fn merge_short_silences(tl: &Timeline, max_gap_s: f32) -> Timeline {
    ...
    for seg in &tl.segments {
        match merged.last_mut() {
            Some(prev) if prev.kind == SegmentKind::Voiced
            ...
            Some(prev) if prev.kind == SegmentKind::Voiced
                && seg.kind == SegmentKind::Voiced =>
            {
                // Merge consecutive voiced runs (shouldn't normally happen but be safe)
                prev.end_s      = seg.end_s;
                prev.duration_s = prev.end_s - prev.start_s;
                // Update RMS: take max (loudest is most representative)
                if seg.rms_db > prev.rms_db { prev.rms_db = seg.rms_db; }
            }
            _ => merged.push(seg.clone()),
        }
    }

    let pause_count = merged.iter()
        .filter(|s| s.kind == SegmentKind::Silence && s.duration_s >= PAUSE_MIN_S)
        .count();
    let total_silence_s: f32 = merged.iter()
        .filter(|s| s.kind == SegmentKind::Silence)
        .map(|s| s.duration_s)
        .sum();
    let silence_ratio = if tl.total_duration_s > 0.0 { total_silence_s / tl.total_duration_s } else { 0.0 };
...
mod tests {
    ...
    fn silence(secs: f32, sr: u32) -> Vec<f32> { vec![0.0f32; (sr as f32 * secs) as usize] }
    ...
    fn short_gap_below_threshold_not_a_pause() {
        ...
        s.extend(tone(440.0, 0.4, 22050));
        let tl = segment(&s, 22050);
        let long_pauses: Vec<_> = pauses(&tl).into_iter()
