...
fn mel_filterbank(n_fft: usize, sr: u32) -> Vec<Vec<f32>> {
    ...
    let pts: Vec<f32> = (0..=N_MELS + 1)
        ...
        .collect();
    let bins: Vec<usize> = pts.iter()
        .map(|&m| ((n_fft + 1) as f32 * mel_to_hz(m) / sr).floor() as usize)
        .collect();
    (0..N_MELS).map(|m| {
        let mut f = vec![0.0f32; n_fft / 2];
        for k in bins[m]..bins[m+1] { if k < f.len() { f[k] = (k - bins[m]) as f32 / (bins[m+1] - bins[m]) as f32; } }
        for k in bins[m+1]..bins[m+2] { if k < f.len() { f[k] = (bins[m+2] - k) as f32 / (bins[m+2] - bins[m+1]) as f32; } }
        f
    }).collect()
}
