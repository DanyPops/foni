...
fn default_duration_ms()  -> u32 { 120 }
...
pub async fn breath(
    ...
    Ok(Json(BreathResponse { audio_data: B64.encode(&out) }))
}
...
mod tests {
    ...
    fn generate_breath_samples(dur_ms: u32, sr: u32) -> Vec<f32> {
        ...
        for i in 0..fade_in  { samples[i]         *= i as f32 / fade_in  as f32; }
        for i in 0..fade_out { samples[n-1-i]     *= i as f32 / fade_out as f32; }
        samples
    }
