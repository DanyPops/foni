...
use foni_analyse::{
    analyse as run_analyse, compute_gap, decode_wav,
    AnalysisResult, GapResult, TargetTensor,
};
...
pub async fn analyse(
    ...
    let ref_bytes = req.reference_data.as_deref()
        .map(|b64| B64.decode(b64))
        .transpose()
        .map_err(|e| (StatusCode::BAD_REQUEST, format!("reference base64: {e}")))?;

    let ref_wav = ref_bytes.as_deref()
        .map(decode_wav)
        .transpose()
        .map_err(|e| (StatusCode::UNPROCESSABLE_ENTITY, format!("reference wav: {e}")))?;

    let label  = req.reference_label.unwrap_or_else(|| "reference".to_string());
    ...
    Ok(Json(AnalyseResponse { analysis: result.0, gap_result: result.1 }))
}
